...
import { Check, ArrowRight, BarChart3, Zap, TrendingUp, Lock } from 'lucide-react';
import { useContactForm } from '@/contexts/ContactFormContext';

export default function BusinessProcessAutomationPage() {
  const { openContactForm } = useContactForm();
  const benefits = ['30-50% cost reduction', 'Faster process completion', '70%+ error reduction', 'Scalable without hiring', 'Improved compliance', 'Better decision-making'];
  const features = [
    { icon: BarChart3, title: 'Process Design', description: 'Intelligent BPA architecture', color: 'from-blue-500 to-blue-600' },
    { icon: Zap, title: 'Smart Execution', description: 'Autonomous process operation', color: 'from-indigo-500 to-indigo-600' },
    { icon: TrendingUp, title: 'Continuous Optimization', description: 'Ongoing improvement cycles', color: 'from-orange-500 to-orange-600' },
    { icon: Lock, title: 'Enterprise Compliance', description: 'Audit-ready automation', color: 'from-green-500 to-green-600' },
  ];
  const faqs = [
    { question: 'What processes can we automate?', answer: 'Any repetitive business process: accounts payable, procurement, data processing, customer onboarding, claims handling, and more.' },
    { question: 'How long to implement BPA?', answer: 'Most projects take 6-12 weeks from analysis to full production deployment.' },
    { question: 'What ROI should we expect?', answer: 'Typical ROI appears within 3-6 months with 200-400% annual return on investment.' },
    { question: 'Can BPA work with legacy systems?', answer: 'Yes, our approach bridges legacy and modern systems without modifications.' },
  ];
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
          <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <motion.div className="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-full px-4 py-2 mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              ...
              <span className="text-sm font-medium text-blue-700">Strategic BPA • Operational Efficiency • Enterprise ROI</span>
            </motion.div>
            ...
            <motion.p className="text-lg text-slate-600 mb-6 leading-relaxed max-w-3xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.25 }}>
              Strategic automation consulting to transform business processes, reduce costs, and improve operational efficiency across your enterprise.
            </motion.p>
      ...
      <section className="py-16 md:py-24 bg-gradient-to-r from-blue-900 via-slate-900 to-blue-900 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-6">Transform Your Processes</h2>
