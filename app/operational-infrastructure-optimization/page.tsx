...
import { Check, ArrowRight, Cpu, Zap, TrendingUp, Lock } from 'lucide-react';
import { useContactForm } from '@/contexts/ContactFormContext';

export default function OperationalInfrastructurePage() {
  const { openContactForm } = useContactForm();
  const benefits = ['Cloud-native architecture', '99.99% uptime SLA', 'Automatic scaling', 'Enterprise security', 'Cost optimization', 'AI-ready infrastructure'];
  const features = [
    { icon: Cpu, title: 'Architecture Design', description: 'Scalable infrastructure architecture', color: 'from-blue-500 to-blue-600' },
    { icon: Zap, title: 'Performance', description: 'Optimized for high-performance operations', color: 'from-indigo-500 to-indigo-600' },
    { icon: TrendingUp, title: 'Scalability', description: 'Effortless scaling as you grow', color: 'from-orange-500 to-orange-600' },
    { icon: Lock, title: 'Security', description: 'Enterprise-grade security infrastructure', color: 'from-green-500 to-green-600' },
  ];
  const faqs = [
    { question: 'What infrastructure do you support?', answer: 'AWS, Azure, Google Cloud, on-premise, hybrid, and multi-cloud deployments.' },
    { question: 'How do you ensure reliability?', answer: 'Multi-region deployment, load balancing, auto-recovery, and comprehensive monitoring.' },
    { question: 'What about compliance and security?', answer: 'Full SOC2, GDPR, HIPAA compliance with encryption, access controls, and audit trails.' },
    { question: 'Can you optimize existing infrastructure?', answer: 'Yes, we audit and optimize current setups for performance and cost efficiency.' },
  ];
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
          <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <motion.div className="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-full px-4 py-2 mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              ...
              <span className="text-sm font-medium text-blue-700">Cloud Architecture • Enterprise Reliability • Scalability</span>
            </motion.div>
            ...
            <motion.p className="text-lg text-slate-600 mb-6 leading-relaxed max-w-3xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.25 }}>
              Build scalable, AI-ready operational infrastructure optimized for performance, reliability, and security at enterprise scale.
            </motion.p>
      ...
      <section className="py-16 md:py-24 bg-gradient-to-r from-blue-900 via-slate-900 to-blue-900 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-6">Optimize Your Infrastructure</h2>
